...
class Game {
  ...
  generateNappy() {
    ...
    const colStart = Math.floor(Math.random() * 12);
    ...
    var nappyIntervalId = setInterval(() => {
      ...
      if(parseFloat(nappy.style.gridRow) >= 11 || 
  ...
  moveNappy(nappy) {
    ...
    //Collision avec MOM
    //Action '-' sur setEnergyBar
    this.nappies.forEach(nappy => {
      if(nappy.style.gridRow === 10 && nappy.style.gridColumn === this.currentPosition) 
      console.log("bim bam boum")
    })
      // this.setEnergyBar() -= 10/100;
  };

// detectCollision(){
//     this.nappies.forEach(nappy => {
//       if(nappy.style.gridRow === 10 && nappy.style.gridColumn === mom.style.gridColumn) 
//       console.log("bim bam boum")
//     })
//     this.coffees.forEach(coffee => {
//       if(coffee.style.gridRow === 10 && coffee.style.gridColumn === this.mom.style.gridColumn)
//       console.log("aie")
//     })
  ...
  let coffeeIntervalId = setInterval(() => {
      this.moveCoffee(coffee);
        ...
        //suppression de l'objet lorsqu'il sort de la grid 
        //OU lorsqu'il est à la même position que l'objet MOM
      if(parseFloat(coffee.style.gridRow)>=11
  ...
  moveMom(direction) {
    ...
    if (direction === "right" && this.currentPosition + 1 < 11) {
      mom.style.gridColumn = `${this.currentPosition + 1} / ${this.currentPosition + 2}`;

    } else if (direction === "left" && this.currentPosition - 1 > 0) {
      mom.style.gridColumn = `${this.currentPosition - 1} / ${this.currentPosition}`;
    }
  }
  ...
  setEnergyBar(value) {
    ...
    else if(this.energyBar <70 && this.energyBar >= 40) bar.classList.add("warning");
